...
@Field String svcName            = "optimization-engine"
...
properties([
    parameters([
        [$class: 'ChoiceParameter',
            ...
            filterable: true,
            name: 'branch',
            randomName: 'choice-parameter-oe-branch',
            script: [
                $class: 'GroovyScript',
...
node("RETB-slv101") {
    ...
    println("===========================================================================================")
    println("# Build Optimization-Engine")
    ...
    try{
        ...
        dir("Backend") {
            ...
            stage("Extract SO Version from CSProj") {
                println("# Extracting SO version from csproj (.so file)")
                csprojPath = ado_library.findCsprojFile(svcName)
                fileContent = readFile(file: csprojPath, encoding: 'UTF-8')
                soVersion = ado_library.extractSoVersion(fileContent)
                println("# SO Version: ${soVersion}")
            }
            ...
            stage("Set Chart And CSProj Version") {
                ...
                newVersion = ado_library.updateTwoPartsVersion("Chart.yaml", soVersion)
